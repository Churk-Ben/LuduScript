...
run_tests() {
    ...
    if [ -f "$BIN_DIR/luduscript" ]; then
        "$BIN_DIR/luduscript" scripts/generate_poker_deck.gen --output output/test_poker.json
        "$BIN_DIR/luduscript" scripts/generate_werewolf_cards.gen --output output/test_werewolf.json
        echo "测试完成！"
    else
